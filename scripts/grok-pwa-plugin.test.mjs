...
const EMPTY_CWD = mkdtempSync(join(tmpdir(), "grok-og-empty-cwd-"));
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, { appName: "Wild Race", cwd: EMPTY_CWD });
...
test("injects into documents with no head element", () => {
  const out = injectGrokPwaHead("<html><body>hi</body></html>", { appName: "Solo", cwd: EMPTY_CWD });
...
test("uses the app name in the injected title tag", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", { appName: "Wild Race", cwd: EMPTY_CWD });
