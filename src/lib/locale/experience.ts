...
export const experienceChrome = {
  zh: {
    ...
    timelineKeyboard: "左右鍵換幀，空白鍵播放，O 切 onion-skin，C 比較，R 只修問題幀。",
    demoMark: "示範",
    ...
    processOperate: "操作這個節點",
    processFakeGen: "假生成（無金鑰）",
    processCreate: "建立示範專案",
    processWorld: "寫入世界觀",
    processPickAsset: "選入素材庫",
    processApprove: "通過",
    processReject: "退回",
    processPack: "打包交付",
    processPacked: "示範交付包已標好，不是真實分享連結。",
    processShotUp: "上移鏡頭",
    processShotDown: "下移鏡頭",
    playTimeline: "播放",
    pauseTimeline: "暫停",
    repairHere: "只修這一窗",
    repairedNote: "已用鄰幀線性混合修過（示範，不是 GPU）",
    heatmapToggle: "熱圖",
    calcLimits: "瀏覽器只做約 160px 降採樣的亮度／對比／邊緣帶。沒有 OpenCV、YuNet 或眼動儀。",
    regionSelect: "點區域看推估值",
    view2d: "俯視",
    viewIso: "等角",
    circulationAria: "示意動線",
    annotationLayer: "註記層",
    deletePin: "刪除註記",
    selectedPin: "選取註記",
    hermesReady: "探測就緒",
    hermesReadyResult: "尚未連線 · GitHub 網址不是 MCP",
    hermesMcpNote: "真實 HTTPS endpoint 與 token 才探測。本頁不會連出去。",
    folioLiveHint: "同一份示範文件：畫布可放文字／形狀，畫板共用，指令會寫入；MCP 步驟預設 dry-run。",
    folioDryRun: "dry-run · 未寫入",
    folioApplied: "已寫入示範文件",
    zenBreathe: "跟著呼吸",
    folioBack: "回到文件櫃",
    folioPublish: "發布",
    folioSaved: "暫存",
  ...
  en: {
    ...
    timelineKeyboard: "Left and right change frames. Space plays. O toggles onion-skin. C compares. R repairs a problem frame.",
    demoMark: "Demo",
    ...
    processOperate: "Operate this node",
    processFakeGen: "Fake generate (no key)",
    processCreate: "Create demo project",
    processWorld: "Write worldview",
    processPickAsset: "Add to asset library",
    processApprove: "Approve",
    processReject: "Return",
    processPack: "Pack for delivery",
    processPacked: "A labeled demo pack — not a real share link.",
    processShotUp: "Move shot up",
    processShotDown: "Move shot down",
    playTimeline: "Play",
    pauseTimeline: "Pause",
    repairHere: "Repair this window only",
    repairedNote: "Linear-blended from neighbors (demo, not GPU)",
    heatmapToggle: "Heatmap",
    calcLimits: "The browser only downsamples to about 160px for luma / contrast / edge bands. No OpenCV, YuNet, or eye-tracking.",
    regionSelect: "Tap a region for the estimate",
    view2d: "Plan",
    viewIso: "Iso",
    circulationAria: "Sketched circulation",
    annotationLayer: "Annotation layer",
    deletePin: "Delete note",
    selectedPin: "Selected note",
    hermesReady: "Probe ready",
    hermesReadyResult: "Not connected · a GitHub URL is not MCP",
    hermesMcpNote: "Only a real HTTPS endpoint and token are probed. This page does not call out.",
    folioLiveHint: "One demo document: canvas can place text/shapes, artboards share it, commands write; the MCP step stays dry-run.",
    folioDryRun: "dry-run · not written",
    folioApplied: "Written to the demo document",
    zenBreathe: "Follow the breath",
...
export const experienceCopyEn: Record<string, ExperienceCopyEn> = {
  ...
  framelab: {
    ...
    fileHints: [
      ...
      { path: "src/lib/domain/repair-planner.ts", purpose: "Repair planner", stage: "Repair" },
  ...
  planform: {
    ...
    fileHints: [
      ...
      { path: "src/core/corridorGeometry.ts", purpose: "Corridor editor geometry", stage: "Venue" },
      { path: "src/core/spatialKnowledge.ts", purpose: "Citable design notes (not a code certificate)", stage: "Limits" },
      { path: "src/core/rehearsal.ts", purpose: "Pre-event rehearsal", stage: "Circulation" },
  ...
  "hermes-console": {
    ...
    conversation: {
      ...
      suggestions: ["Poster", "Connection", "Task", "Ready"],
      replies: [
        {
          matchZh: "海報",
          match: "poster",
        ...
        },
        {
          matchZh: "就緒",
          match: "ready",
          reply: "GET /api/ready is a connection probe. Without HERMES_API_URL the workspace should still open and show not connected. A GitHub URL is not MCP.",
        },
      ],
    },
    walkthrough: [
    ...
    fileHints: [
      ...
      { path: "docs/LUMEN.md", purpose: "Lumen MCP; a GitHub URL is not MCP", stage: "Tools" },
      { path: "docs/FRAMELAB.md", purpose: "FrameLab MCP", stage: "Tools" },
      { path: "docs/DUIGAO.md", purpose: "Review MCP", stage: "Tools" },
      { path: "docs/PLANFORM_MCP.md", purpose: "PLANFORM MCP", stage: "Tools" },
      { path: "docs/ATLAS_MCP.md", purpose: "Atlas MCP; a GitHub URL is not MCP", stage: "Tools" },
    ],
  },
  "tku-zen-ai": {
